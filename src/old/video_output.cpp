...
CVideoOutput::CVideoOutput(CVideoMixer* pVideoMixer)
{
	...
	if (!(m_pVideoMixer && m_pOutputMutex && m_pOutputSem)) exit(1);
}
...
if (verbose) fprintf(stderr, " - Adding %.3lf\n", time2double(time_delta));
		...
		// Compare it for the time for the next frame
		timersub(&next_time, &time_now, &time_delta);
if (verbose) fprintf(stderr, " - timenow %ld.%03ld period %ld.%03ld actual %ld.%03ld nexttime %ld.%03ld Time until next frame %ld.%03ld\n",
...
#ifdef HAVE_MACOSX          // suseconds_t is unsigned integer for OS X
	fprintf(stderr, "Loop Frame period %ld.%06d secs fps %.3lf period %.3lf\n",
#else                       // suseconds_t is long unsigned int for Linux
...
#ifdef HAVE_MACOSX
//#error We should set the socket option SO_NOSIGPIPE and check for EPIPE error
#endif
...
#endif
					(&buffer),
				  sizeof (*cb) + strlen (m_output_memory_handle_name) + 1, MSG_NOSIGNAL);
			if (x > 0) {
...
//fprintf(stderr, "Frame %u sent %d bytes\n", m_buffers_sent, n);
	if (n < 0) {
		...
		if (errno == EAGAIN) {
			...
			// Client disappeared.
			fprintf (stderr, "Output receiver dissapered\n"
